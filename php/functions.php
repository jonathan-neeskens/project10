...
function change_table($function, $id, $table_nr){
    ...
    if ($function == 'delete'){
        ...
        $query = mysqli_query($link, "UPDATE `tables` SET `table_nr` = '$table_nr' WHERE `id` = '$id'");
        header("Location: table_settings.php?change=success");
    }
}
...
function create_menu($name, $price){
    ...
    $query = mysqli_query($link, "INSERT INTO `menu` VALUES (NULL, '$name', '$price', '1', '$randcolor')");
...
function menu_list(){
    ...
    while($row1 = mysqli_fetch_assoc($query1)){
    $color = "background-color: ".$row1['app_color'];
    $img = 'background-image: url("img/menu.png")';
        echo "
        <div class='list_item'>
            <div class='user_pic' style='".$img."; ".$color."')'>
            </div>
